 ...
 *
 *  @note    Backtracking Strong Wolfe Line Search Implementation used by the L-BFGS Algorithm.
 &
        ...
        while true do
            ...
            if fNew > f + stpNew * dgtest then
                ...
                if dg < params.wolfe * dginit then
                    width = inc
                else if dg > -params.wolfe * dginit then       // check the strong Wolfe condition
                    width = dec
                else                                           // exit with the strong Wolfe condition
