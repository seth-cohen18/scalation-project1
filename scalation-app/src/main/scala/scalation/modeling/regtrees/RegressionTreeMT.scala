 ...
 *  @note:  current does not implement feature bagging, nor use the parameter `use_r_fb`
 ...
 *  @param use_r_fb     whether to use feature bagging (select subsets of the features)
...
class RegressionTreeMT (x: MatrixD, y: VectorD, fname_ : Array [String] = null,
    ...
    debug ("init", s"use_r_fb = $use_r_fb")
...
//  println (s"y = $y")
    ...
    for tech <- SelectionTech.values do
        ...
        new PlotM (null, rSq, Regression.metrics, s"R^2 vs n for Regression Tree with $tech", lines = true)
