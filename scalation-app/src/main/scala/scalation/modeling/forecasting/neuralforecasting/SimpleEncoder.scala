...
package scalation.modeling.forecasting.neuralforecasting
...
object SimpleEncoder:
     ...
     *  An affine transformation is supported via the γ and β learnable parameters.
     *  @param x  the matrix to normalize
     *  @param γ  the scaling learnable parameter (defaults to 1.0)
     *  @param β  the shifting learnable parameter (defaults to 0.0)
     */
    def layerNorm (x: MatrixD, γ: Double = 1.0, β: Double = 0.0): MatrixD = 
        val xt = x.ᵀ                                          // transpose since ScalaTion computes means and stdevs column-wise
//      val xn = ((xt - xt.mean) / (xt.stdev + eps)).ᵀ        // z-transform to mean zero and stdev one
        val xn = ((xt - xt.mean) / (xt.variance + eps).sqrt).ᵀ   // z-transform to mean zero and stdev one
        xn * γ + β                                            // allows scaling and shifting for flexibility
    end layerNorm
...
@main def simpleEncoder2 (): Unit =
    ...
    import scalation.modeling.neuralnet.SimpleCNN.{y => yy}   // example univariate time series of length 20 (t = 0 ... 19)
...
//  val heads = 1                                             // number of attention heads (d_model = d_k * heads)
    ...
    val (μ, σ) = (yy.mean, yy.stdev)
    val y  = (yy - μ) / (σ + eps)                             // normalize the whole time series via z-transformation 
    ...
    x  = layerNorm (x)                                        // apply layer normalization (use default γ and β)
...
//  val z  = f_geLU.fM (u)                                    // hidden matrix from f0 = geLU activation
    ...
    val ŷ  = vv                                               // output/prediction matrix: typically no activation
    ...
    x += ŷ                                                    // add output from FNN to x (residual connection)
    x  = layerNorm (x)                                        // apply layer normalization a second time (use default γ and β)
    ...
    val ŷ_norm = h_last ∙ w_out + b_out                       // scalar on normalized (z) scale
    val ŷ_h1   = μ + (σ + eps) * ŷ_norm                       // apply back-transformation to get forecast

    println (s"forecast (normalized)     = $ŷ_norm")
    println (s"forecast (original scale) = $ŷ_h1")

    val ε = y_h1 - ŷ_h1                                       // error at horizon 1 (actual - forecasted)

    println (s"forecast error at time t = ${y.dim}: ε = $ε")
