...
object Integral:
    ...
    private val debug  = debugf ("Integral", true)         // debug function
    ...
    def simpson (a: Double, b: Double, f: FunctionS2S, sd: Int = SUBDIV): Double =
        ...
        val dx = (b - a) / sd
        var sum = f(a) + f(b)
        for i <- 1 until sd do { x += dx; sum += c_simp (i % 2) * f(x) }
        _1_3 * dx * sum
    end simpson
    ...
    def simpson38 (a: Double, b: Double, f: FunctionS2S, sd: Int = SUBDIV): Double =
        ...
        val dx = (b - a) / sd
        var sum = f(a) + f(b)
        for i <- 1 until sd do { x += dx; sum += c_sp38 (i % 3) * f(x) }
        _3_8 * dx * sum
    end simpson38
    ...
    def boole (a: Double, b: Double, f: FunctionS2S, sd: Int = SUBDIV): Double =
        ...
        val dx = (b - a) / sd
        var sum = 7.0 * (f(a) + f(b))
        for i <- 1 until sd do { x += dx; sum += c_bool (i % 4) * f(x) }
        _2_45 * dx * sum
    end boole
    ...
    def romberg (a: Double, b: Double, f: FunctionS2S, iter: Int = ITER): Double =
        ...
        for k <- 0 until iter do
            t(k)(0) = trap (a, b, f, sd)                  // use trapezoidal method
            for j <- 1 to k do t(k)(j) = (_4up(j) * t(k)(j-1) - t(k-1)(j-1)) / (_4up(j) - 1)
            debug ("romberg", s"(t(k) = ${stringOf (t(k))}")
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `integralTest2` main function tests the numerical integrators on simple problems.
 *  Easy problems.
 *  > runMain scalation.calculus.integralTest
 */
@main def integralTest (): Unit =
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `integralTest2` main function tests the numerical integrators using the Gauss
 *  function that has no analytic solution.  Hard problem.
 *  > runMain scalation.calculus.integralTest2
 */
@main def integralTest2 (): Unit =
