...
//      def p_ensureSize (n: Int) = super.ensureSize (n)
    ...
    private val debug  = debugf ("PriorityQueueFW", false)  // debug function
...
//      resarr.p_ensureSize (resarr.p_size0 + 1)
    ...
    def printInOrder (): Unit =
        ...
        val pq_ = clone
        while ! pq_.isEmpty do println (pq_.dequeue ())
    end printInOrder
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Return the top (highest priority) up to 3 elements in order from this priority queue.
     */
    def head3: (A, A, A) =
        resarr.p_size0 match
            case 0 => throw new NoSuchElementException ("queue is empty")
            case 1 => (toA (resarr.p_array(1)), null.asInstanceOf [A], null.asInstanceOf [A])
            case 2 => (toA (resarr.p_array(1)), toA (resarr.p_array(2)), null.asInstanceOf [A])
            case _ => val (e2, e3) = (toA (resarr.p_array(2)), toA (resarr.p_array(3)))
                      if ord.gt (e2, e3) then (toA (resarr.p_array(1)), e2, e3)
                      else (toA (resarr.p_array(1)), e3, e2)
    end head3
...
//      val copied = IterableOnce.elemsToCopyToArray (length, xs.length, start, len)
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Use Default Serialization Proxy for writing in place.
     */
...
@main def priorityQueueFWTest (): Unit =
    ...
    println ("Insert 20")
    pq.enqueue (20)
    println (s"pq       = $pq")                            // the whole priority queue
    println (s"pq.head3 = ${pq.head3}")                    // the top 3 in order
