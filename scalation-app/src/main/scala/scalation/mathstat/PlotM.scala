...
//  private val maxY     = ceil (y_.mmax + EPSILON)
     ...
     *  @param x_     the x vector of data values (horizontal)
     *  @param y_     the y VEC of data values where y(i) is the i-th vector (vertical)
     *  @param label  the label/legend/key for each curve in the plot
     *  @param title  the title of the plot
     *  @param lines  flag for generating a line plot
     */
    def this (x_ : VectorD, y_ : VEC [VectorD], label: Array [String], title: String, lines: Boolean) =
        this (x_, MatrixD (y_).ᵀ, label, title, lines)
    end this
