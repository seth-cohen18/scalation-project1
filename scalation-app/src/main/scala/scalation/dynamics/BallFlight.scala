...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `ballFlight` main function is used to illustrate the `RungeKutta` 'RK' and
...
//  val m  =  45.93                                              // mass of a golf ball in grams
    ...
    breakable {
        for i <- 1 to n do
            ...
            if p_e(i, 1) < 0.0 then { p_e(i, 1) = 0; break () }  // quit after hitting the ground
            ...
            println (s"> at t = ${"%4.1f".format(t)}, p_r = ${p_r(i)}, p_d = ${p_d(i)}, p_e = ${p_e(i)}")
            tt(i) = t
            t += dt
        end for
    } // breakable

    new Plot (tt, p_r(?, 0), p_d(?, 0), "Plot x vs. t (black-RK, red-DP)")
    new Plot (tt, p_r(?, 1), p_d(?, 1), "Plot y vs. t (black-RK, red-DP)")
    new Plot (tt, p_r(?, 1), p_e(?, 1), "Plot y vs. t (black-RK, red-EX)")
    new Plot (tt, p_d(?, 1), p_e(?, 1), "Plot y vs. t (black-DP, red-EX)")
    new Plot (p_d(?, 0), p_d(?, 1))
